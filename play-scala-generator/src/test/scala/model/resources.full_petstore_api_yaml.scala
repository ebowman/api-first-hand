...
object full_petstore_api_yaml extends WithModel {
 ...
 def types = Map[Reference, Type](
	...
	Reference("⌿definitions⌿Pet") → 
		TypeDef(Reference("⌿definitions⌿Pet"), 
			Seq(
					...
					Field(Reference("⌿definitions⌿Pet⌿tags"), Opt(ArrResult(TypeRef(Reference("⌿definitions⌿Tag")), TypeMeta(None, List())), TypeMeta(None, List()))),
					Field(Reference("⌿definitions⌿Pet⌿category"), Opt(TypeRef(Reference("⌿definitions⌿Tag")), TypeMeta(None, List())))
			), TypeMeta(Some("Named types: 6"), List())),
	...
	Reference("⌿paths⌿/users/createWithList⌿post⌿body") → 
		Opt(ArrResult(TypeRef(Reference("⌿definitions⌿User")), TypeMeta(None, List())), TypeMeta(None, List())),
