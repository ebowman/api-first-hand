    ...
    "GET /api/" should {
        def testInvalidInput(activity: Activity): Prop = {
            ...
            val propertyList = contentHeaders.map { case (consumes, produces) =>
                ...
                (s"given 'Content-Type' [$consumes], 'Accept' header [$produces] and URL: [$url]" + " and body [" + parsed_activity + "]") |: all(
        ...
        def testValidInput(activity: Activity): Prop = {
            ...
            val propertyList = contentHeaders.map { case (consumes, produces) =>
                ...
                (s"Given response code [$expectedCode], 'Content-Type' [$consumes], 'Accept' header [$produces] and URL: [$url]" + " and body [" + parsed_activity + "]") |: all(
    ...
    "POST /api/another" should {
        def testInvalidInput(example: AnotherPostExample): Prop = {
            ...
            val propertyList = contentHeaders.map { case (consumes, produces) =>
                ...
                (s"given 'Content-Type' [$consumes], 'Accept' header [$produces] and URL: [$url]" + " and body [" + parsed_example + "]") |: all(
                    "StatusCode = BAD_REQUEST" |: (requestStatusCode_(path) ?= BAD_REQUEST),
                    s"Content-Type = $produces" |: ((consumes ?= "*/*") || (requestContentType_(path) ?= Some(produces))),
                    "non-empty errors" |: (errors.nonEmpty ?= true),
                    "at least one validation failing" |: atLeastOne(validations:_*)
        ...
        def testValidInput(example: AnotherPostExample): Prop = {
            ...
            val propertyList = contentHeaders.map { case (consumes, produces) =>
                ...
                (s"Given response code [$expectedCode], 'Content-Type' [$consumes], 'Accept' header [$produces] and URL: [$url]" + " and body [" + parsed_example + "]") |: all(
                    "Response Code is allowed" |: (possibleResponseTypes.contains(expectedCode) ?= true),
                    "Successful" |: (parsedApiResponse.isSuccess ?= true),
                    s"Content-Type = $produces" |: ((parsedApiResponse.get ?= null) || (consumes ?= "*/*") || (requestContentType_(path) ?= Some(produces))),
                    "No errors" |: (errors.isEmpty ?= true)
