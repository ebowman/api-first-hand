    ...
    "GET /api/" should {
        def testInvalidInput(basic: Basic): Prop = {
            ...
            val propertyList = contentHeaders.map { case (consumes, produces) =>
                ...
                (s"given 'Content-Type' [$consumes], 'Accept' header [$produces] and URL: [$url]" + " and body [" + parsed_basic + "]") |: all(
        ...
        def testValidInput(basic: Basic): Prop = {
            ...
            val propertyList = contentHeaders.map { case (consumes, produces) =>
                ...
                (s"Given response code [$expectedCode], 'Content-Type' [$consumes], 'Accept' header [$produces] and URL: [$url]" + " and body [" + parsed_basic + "]") |: all(
