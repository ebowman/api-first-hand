    ...
    "GET /api/" should {
        def testInvalidInput(nestedObject: NestedObjects): Prop = {
            ...
            val propertyList = contentHeaders.map { case (consumes, produces) =>
                ...
                (s"given 'Content-Type' [$consumes], 'Accept' header [$produces] and URL: [$url]" + " and body [" + parsed_nestedObject + "]") |: all(
        ...
        def testValidInput(nestedObject: NestedObjects): Prop = {
            ...
            val propertyList = contentHeaders.map { case (consumes, produces) =>
                ...
                (s"Given response code [$expectedCode], 'Content-Type' [$consumes], 'Accept' header [$produces] and URL: [$url]" + " and body [" + parsed_nestedObject + "]") |: all(
