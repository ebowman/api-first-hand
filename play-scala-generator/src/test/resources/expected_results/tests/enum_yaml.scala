...
class Enum_yamlSpec extends WordSpec with OptionValues with WsScalaTestClient with OneAppPerTest  {
  ...
  def parseResponseContent[T](mapper: ObjectMapper, content: String, mimeType: Option[String], expectedType: Class[T]) =
    ...
    "GET /test" should {
        def testInvalidInput(includes: Option[TestGetIncludesOptionEnum]): Prop = {


            val url = s"""/test?${toQuery("includes", includes)}"""
            ...
            val propertyList = contentHeaders.map { case (acceptHeader, contentType) =>
                ...
                val errors = new TestGetValidator(includes).errors
        ...
        def testValidInput(includes: Option[TestGetIncludesOptionEnum]): Prop = {
        ...
        "discard invalid data" in {
            val genInputs = for {
                    includes <- OptionTestGetIncludesOptionEnumGenerator
                } yield includes
            val inputs = genInputs suchThat { includes =>
                new TestGetValidator(includes).errors.nonEmpty
            }
        ...
        "do something with valid data" in {
            ...
            val inputs = genInputs suchThat { includes =>
                new TestGetValidator(includes).errors.isEmpty
            }
