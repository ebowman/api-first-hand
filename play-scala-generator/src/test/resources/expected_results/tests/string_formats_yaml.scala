    ...
    "GET /" should {
        def testInvalidInput(input: (Option[ZonedDateTime], Option[LocalDate], Option[Base64String], Option[UUID], BinaryString)): Prop = {
            ...
            val propertyList = contentHeaders.map { case (consumes, produces) =>
                ...
                (s"given 'Content-Type' [$consumes], 'Accept' header [$produces] and URL: [$url]" + " and body [" + parsed_petId + "]") |: all(
        ...
        def testValidInput(input: (Option[ZonedDateTime], Option[LocalDate], Option[Base64String], Option[UUID], BinaryString)): Prop = {
            ...
            val propertyList = contentHeaders.map { case (consumes, produces) =>
                ...
                (s"Given response code [$expectedCode], 'Content-Type' [$consumes], 'Accept' header [$produces] and URL: [$url]" + " and body [" + parsed_petId + "]") |: all(
