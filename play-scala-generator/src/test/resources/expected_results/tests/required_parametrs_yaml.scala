...
class Required_parametrs_yamlSpec extends WordSpec with OptionValues with WsScalaTestClient with OneAppPerTest  {
  ...
  def parseResponseContent[T](mapper: ObjectMapper, content: String, mimeType: Option[String], expectedType: Class[T]) =
    ...
    "GET /" should {
        def testInvalidInput(input: (String, GetTest2)): Prop = {

            val (test1, test2) = input

            val url = s"""/?${toQuery("test1", test1)}"""
            ...
            val propertyList = contentHeaders.map { case (acceptHeader, contentType) =>
                val headers =
                    ...
                    val parsed_test2 = PlayBodyParsing.jacksonMapper("application/json").writeValueAsString(test2)

                val request = FakeRequest(GET, url).withHeaders(headers:_*).withBody(parsed_test2)
                val path =
                ...
                val errors = new GetValidator(test1, test2).errors
                ...
                (s"given 'Content-Type' [$contentType], 'Accept' header [$acceptHeader] and URL: [$url]" + " and body [" + parsed_test2 + "]") |: all(
        ...
        def testValidInput(input: (String, GetTest2)): Prop = {
            val (test1, test2) = input
            
            val parsed_test2 = parserConstructor("application/json").writeValueAsString(test2)
            ...
            val propertyList = contentHeaders.map { case (acceptHeader, contentType) =>
                ...
                (s"Given response code [$expectedCode], 'Content-Type' [$contentType], 'Accept' header [$acceptHeader] and URL: [$url]" + " and body [" + parsed_test2 + "]") |: all(
        ...
        "discard invalid data" in {
            val genInputs = for {
                        test1 <- StringGenerator
                        test2 <- GetTest2Generator
                    
                } yield (test1, test2)
            val inputs = genInputs suchThat { case (test1, test2) =>
                new GetValidator(test1, test2).errors.nonEmpty
            }
        ...
        "do something with valid data" in {
            ...
            val inputs = genInputs suchThat { case (test1, test2) =>
                new GetValidator(test1, test2).errors.isEmpty
            }
