...
class Foodpanda_yamlSpec extends WordSpec with OptionValues with WsScalaTestClient with OneAppPerTest  {
  ...
  def parseResponseContent[T](mapper: ObjectMapper, content: String, mimeType: Option[String], expectedType: Class[T]) =
    ...
    "GET /api/vendors" should {
        def testInvalidInput(query: VendorQuery): Prop = {


            val url = s"""/api/vendors"""
            ...
            val propertyList = contentHeaders.map { case (acceptHeader, contentType) =>
                val headers =
                    ...
                    val parsed_query = PlayBodyParsing.jacksonMapper("application/json").writeValueAsString(query)

                val request = FakeRequest(GET, url).withHeaders(headers:_*).withBody(parsed_query)
                val path =
                ...
                val errors = new VendorsGetValidator(query).errors
                ...
                (s"given 'Content-Type' [$contentType], 'Accept' header [$acceptHeader] and URL: [$url]" + " and body [" + parsed_query + "]") |: all(
        ...
        def testValidInput(query: VendorQuery): Prop = {
            
            val parsed_query = parserConstructor("application/json").writeValueAsString(query)
            ...
            val propertyList = contentHeaders.map { case (acceptHeader, contentType) =>
                ...
                (s"Given response code [$expectedCode], 'Content-Type' [$contentType], 'Accept' header [$acceptHeader] and URL: [$url]" + " and body [" + parsed_query + "]") |: all(
        ...
        "discard invalid data" in {
            val genInputs = for {
                    query <- VendorQueryGenerator
                } yield query
            val inputs = genInputs suchThat { query =>
                new VendorsGetValidator(query).errors.nonEmpty
            }
        ...
        "do something with valid data" in {
            ...
            val inputs = genInputs suchThat { query =>
                new VendorsGetValidator(query).errors.isEmpty
            }
